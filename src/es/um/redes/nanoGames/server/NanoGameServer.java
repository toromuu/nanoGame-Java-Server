...
import es.um.redes.nanoGames.server.roomManager.NGConnect4Manager;
import es.um.redes.nanoGames.server.roomManager.NGCountSymbolsManager;
...
public class NanoGameServer implements Runnable {
    ...
    private NanoGameServer(InetSocketAddress a)
    {
    	...
    	short idSala = 1;
    	manager.registerRoomManager(new NGConnect4Manager(idSala));
    	idSala++;
    	manager.registerRoomManager(new NGConnect4Manager(idSala));
    	idSala++;
    	manager.registerRoomManager(new NGCountSymbolsManager(idSala));
    	idSala++;
    	manager.registerRoomManager(new NGCountSymbolsManager(idSala));
